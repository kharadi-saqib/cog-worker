...
class CogGeneratorWorker(BaseWorker):
    ...
    """def folder_space_info(self, folder_path):
        log(
            "Reached at the “folder_space_info” Method & About to Execute.",
            level=logging.DEBUG,
        )

        # It will show the total ,used and free space of whole disk.
        total, used, free = shutil.disk_usage(folder_path)

        total_mb = total / (1024**2)
        log(f"Total Capacity of disk is: {total_mb}.", level=logging.INFO)

        used_mb = used / (1024**2)
        log(f"Total Used Capacity of disk is: {used_mb}.", level=logging.INFO)

        free_mb = free / (1024**2)
        log(f"Total Free Capacity of disk is: {used_mb}.", level=logging.INFO)

        return {
            "TotalCapacity": total_mb,
            "UsedCapacity": used_mb,
            "FreeCapacity": free_mb,
        }"""
        ...
        for i, image in enumerate(target_images):
            ...
            if not image.IsCOGGenerated:
                ...
                if settings.GENERATE_COG_ON_LOCAL == "YES":
                    ...
                    # log(
                    #     "Generating NAS Drive tiff path.",
                    #     source_data=source_data,
                    #     target_data=target_data,
                    #     level=logging.DEBUG,
                    # )
                    ...
                    base_path = settings.SERVER_PATH

                    # log(
                    #     f"Server path is: {base_path}",
                    #     source_data=source_data,
                    #     target_data=target_data,
                    #     level=logging.DEBUG,
                    # )
                    ...
                    # log(
                    #     f"Formatted month_year: {month_year}",
                    #     source_data=source_data,
                    #     target_data=target_data,
                    #     level=logging.DEBUG,
                    # )
                    ...
                    # log(
                    #     f"New directory path: {new_directory_path}",
                    #     source_data=source_data,
                    #     target_data=target_data,
                    #     level=logging.DEBUG,
                    # )
                    ...
                    # log(
                    #     f"NAS Drive COG path is: {new_nas_cog_path}",
                    #     source_data=source_data,
                    #     target_data=target_data,
                    #     level=logging.DEBUG,
                    # )
